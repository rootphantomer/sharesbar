...
class ThemeSetting extends React.Component {
  ...
  handleChangeComplete = (color, event) => {
    const tmpState =
      this.state.activeColor === "up"
        ? { pickerColor: color.hex, upColor: color.hex }
  ...
  handleColcorClick(event) {
    const pickerColor =
      event.target.title === "up" ? this.state.upColor : this.state.downColor;
    this.setState({
      activeColor: event.target.title,
      display: "block",
      ...
      <>
        <div style={{ textAlign: "center", display: "inline-block" }}>
          <Row className="rowStyle">
            ...
            <Col
              span={7}
              className="colorColStyle"
              style={upColorStyle}
              title="up"
              onClick={this.handleColcorClick}
            ></Col>
            <Col span={3} offset={2}>
              跌
            </Col>
